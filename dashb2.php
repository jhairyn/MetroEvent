...
if ($_SESSION['role'] == 'admin') {
    ...
    echo '</tbody>';
    echo '</table>';
} else // Handle Organizer Role Request
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['org_role'])) {
    // Check if the user has already requested the organizer role
    $checkRequest = $metro_events->prepare("SELECT * FROM organizer_requests WHERE user_id = ?");
    $checkRequest->execute([$user_id]);

    if ($checkRequest->rowCount() > 0) {
        echo '<p class="alert alert-info">You have already requested to become an organizer.</p>';
    } else {
        // Insert the request into the organizer_requests table
        $stmt = $db->prepare("INSERT INTO organizer_requests (user_id, status) VALUES (?, 'pending')");
        if ($stmt->execute([$user_id])) {
            echo '<p class="alert alert-success">Your request to become an organizer has been submitted.</p>';
        } else {
            echo '<p class="alert alert-danger">There was an error submitting your request. Please try again later.</p>';
        }
    }
}
...
// Add the organizer role request form
echo '<form method="POST" action="">'; // Action points to the same page
echo '<button type="submit" class="btn btn-info" name="org_role">Request Organizer Role</button>';
echo '</form>';
